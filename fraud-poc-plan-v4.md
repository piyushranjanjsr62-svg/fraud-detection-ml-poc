...
- Classifies transactions into three action tiers: Approved (GREEN), Review (AMBER), Hold (RED)
...
Indian Business Sender → CrossBorderPay Platform → Risk Scoring Engine → Decision:
  • Score < 40 (Approved · GREEN):  Payment proceeds → NOSTRO (DBS/JP Morgan)
                                     → SWIFT/Fedwire/CHIPS/SEPA/CHAPS → Overseas Beneficiary
  • Score 40–79 (Review · AMBER):   Analyst Review Queue → Approve → Payment Processing
                                     / Reject → STR Filing with FIU-IND
  • Score ≥ 80 (Hold · RED):        Payment held → Analyst Investigation
```
...
| Three-tier action classification: Approved (GREEN) · Review (AMBER) · Hold (RED) | **Must Have** | Operational workflow requirement |
...
7. Assign risk tier: Approved (0–39) | Review (40–79) | Hold (80–100)
...
→ Score Tier Assignment (Approved/GREEN · Review/AMBER · Hold/RED)
...
|---|---|---|
| Approved (GREEN) | 0–39 | Payment proceeds without analyst intervention |
| Review (AMBER) | 40–79 | Payment queued for human decision |
| Hold (RED) | 80–100 | Payment held pending analyst investigation |
...
| 6. SHAP Deep Dive | 5 min | DS | Walk through one Hold (RED) transaction's top-5 SHAP features; explain what each feature means |
| 7. Adversarial Cases | 5 min | DS | Show one legitimate large export scoring Approved (GREEN); one smurfing pattern scoring Hold (RED) |
...
### Scenario 1 — Clear Fraud (Expected: Hold · RED, Score 85–95)
...
### Scenario 2 — Legitimate Transaction (Expected: Approved · GREEN, Score < 30)
...
### Scenario 3 — Dormant Account Reactivation (Expected: Hold · RED, Score 78–88)
...
### Scenario 4 — Smurfing Sequence (Expected: Escalating Review · AMBER → Hold · RED)

```
Three transactions for same customer, USD 24,800 each, 8 minutes apart.

Transaction 1: risk_score ~55 (AMBER · Review)  — velocity_acceleration: moderate
Transaction 2: risk_score ~68 (AMBER · Review)  — txn_count_1h rising
Transaction 3: risk_score ~83 (RED · Hold)      — is_near_threshold=1, txn_count_1h peak
...
|---|---|
| "What happens when the model is wrong and blocks a legitimate transaction?" | "Every Review and Hold transaction goes to an analyst queue — not automatic rejection. The SHAP features tell the analyst exactly what to verify. No transaction is auto-rejected without human review at current threshold settings." |
...
| 4 | **Analyst workflow:** Who reviews Review (AMBER) transactions? What is the SLA? What if no analyst is available? | Head of Risk + Ops | Without this answer, the queue fills and the model's value is unrealised |
...
> 2. The SHAP features for a Hold (RED) transaction make the Head of Risk lean forward and say "yes, that's exactly what I'd want to know"
...
*End of Document — Cross-Border Transaction Risk Scoring Engine PoC Plan V4.0*
